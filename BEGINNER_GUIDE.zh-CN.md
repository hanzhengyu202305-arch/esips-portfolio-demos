...
| 4 | portfolio evidence readiness | `evidenceops-scorecard` | 证据层，证明前三条不是只靠口头包装 |
...
> 我有三个 demo，而且我能用一个 scorecard 检查这些公开证据是否齐全。
...
- `portfolio_evidence_status = PASS` 代表公开证据齐。
- `application_submission_status = NEEDS_OFFICIAL_CONFIRMATION` 代表正式申请还要看当天官方表格。
- EvidenceOps 不是官方 approval，只是公开 portfolio readiness。
...
EvidenceOps Scorecard 检查公开证据和申请边界。
...
```text
EvidenceOps Scorecard checks whether my public portfolio evidence is present, generated, and bounded. It separates reviewer-ready public artifacts from items that still require official application confirmation.
```
