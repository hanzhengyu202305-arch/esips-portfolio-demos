...
| AegisOps answer leakage | A fixture-fed gold label would make diagnosis accuracy meaningless. | Infer from observed evidence and test with a tampered gold label. | [`docs/ADVERSARIAL_REVIEW.md`](ADVERSARIAL_REVIEW.md) |
...
| Kubernetes text-search bypass | Comments, sidecars, or later YAML documents can hide unsafe effective configuration. | Parse YAML structure and inspect every document and container. | [`kube-copilot/reports/adversarial-validation.md`](../kube-copilot/reports/adversarial-validation.md) |
...
| Hidden planning assumptions | Magic constants and invalid battery/map states weaken model credibility. | Expose an `EnergyModel` and reject invalid topology or energy invariants. | [`haul-truck-planner/reports/route-experiment.md`](../haul-truck-planner/reports/route-experiment.md) |
| Self-scoring inflation | Keyword presence can be mistaken for independent quality evidence. | Label the score as completeness and require structured diagnosis, policy, and adversarial outputs. | [`evidenceops-scorecard/reports/evidence-scorecard.md`](../evidenceops-scorecard/reports/evidence-scorecard.md) |
